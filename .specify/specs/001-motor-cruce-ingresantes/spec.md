...
**Created:** 2026-06-16
**Business Context:** constitution.md
**PO:** Samuel Cisneros
...
El motor de cruce automatiza la validación de identidades de los ingresantes de la UNMSM contra la base de datos de la academia Vonex (PostgreSQL). El sistema procesa un CSV (~27,000 filas × 12 columnas) subido por el administrador mediante un job asíncrono en cola Redis. Normaliza los campos y aplica el filtro ALCANZO VACANTE: los registros que lo cumplen se persisten en la tabla ingresantes; los demás, en no_ingresantes. Luego realiza un cruce en dos fases: un match exacto automático y una fase de coincidencia difusa asistida por interfaz React para los cabos sueltos. En paralelo, un módulo independiente gestiona el Catálogo de Áreas y Carreras. Finalmente, genera un reporte consolidado exclusivamente en Excel (por fecha o todas las fechas) con data enriquecida y un dashboard interactivo de analítica.

---

## Architecture & Deployment Context

### Despliegue 100% Intranet (Local)
El sistema está diseñado y concebido exclusivamente para un **despliegue local (entorno de intranet)** sin exposición pública a Internet. 
Debido a esta decisión arquitectónica de confianza de red (Zero-Trust aplicado solo a nivel de input, no perimetral), **se descarta explícitamente la implementación de protecciones de Capa 7** (como Web Application Firewalls - WAF, ModSecurity o CrowdSec) así como aislamientos a nivel de sistema operativo (Chroot, AppArmor). La seguridad de la aplicación se focaliza enteramente en la validación estricta de inputs (sanitización de CSV, validación de Magic Bytes/MIME) y en el principio de mínimo privilegio en la base de datos PostgreSQL, evitando introducir latencia o complejidad innecesaria en la red local.
...
- [ ] **AC-001:** Dado que la UNMSM publica los resultados acumulativos durante las 4 fechas de examen (sin especificar qué día rindió cada postulante), cuando se sube el archivo al sistema, se utiliza el `CODIGO` (único por postulante) como clave de filtro global. El sistema compara los códigos del CSV contra todos los registros ya existentes en la base de datos (`ingresantes` y `no_ingresantes`) y **solo procesa e inserta los `CODIGO`s nuevos**. Los registros procesados en fechas anteriores se ignoran silenciosamente, garantizando que cada lote resultante contenga exclusivamente el delta de nuevos postulantes.
...
- [ ] **AC-004b:** Dado un registro del CSV, cuando el sistema lo procesa para persistencia, entonces sanitiza automáticamente cualquier carácter especial o prefijo de inyección (`=`, `+`, `-`, `@`), garantizando la integridad de la base de datos mediante el uso innegociable de ORM (Prepared Statements) para prevenir cualquier tipo de corrupción de datos o inyección SQL (ver NFR-007).
...
- El filtrado de duplicados (AC-001) compara el campo `CODIGO` de cada fila del CSV contra todos los registros ya existentes en las tablas `ingresantes` y `no_ingresantes`. Solo se procesan e insertan los `CODIGO`s que no existen en ninguna de las dos tablas.
...
**I want** descargar un archivo Excel (`.xlsx`) con el reporte tabular de ingresantes cruzados, enriquecido con datos de la academia y del Catálogo de Áreas y Carreras
**So that** pueda distribuir los resultados del cruce en un formato estandarizado (Contrato de Columnas inmutable), seleccionando entre una fecha específica o el consolidado de todas las fechas
...
- [ ] **AC-014:** Dado un conjunto de lotes procesados, cuando el usuario solicita la descarga del reporte, entonces el sistema le ofrece un selector obligatorio con dos opciones: exportar una `FECHA_EXAMEN` específica o "Todas las fechas" (consolidado). No se permiten exportaciones en otros formatos distintos a Excel (`.xlsx`).
- [ ] **AC-015:** Dado el archivo Excel descargado, entonces la única hoja del reporte cumple estrictamente con el Contrato de Columnas inmutable: columnas A–M para los datos del CSV crudo original y a partir de la columna N los campos enriquecidos del alumno de la academia (Sede, Ciclo, Año académico, Estado resuelto por jerarquía) y los campos enriquecidos del Catálogo de Áreas y Carreras (`AREA_OFICIAL`, `CARRERA_OFICIAL`). Los registros sin match en el catálogo o academia muestran `SIN MAPEAR`. El Excel solo incluye registros con estado `confirmado_automatico` o `confirmado_manual`.

#### Technical Notes
- La exportación es responsabilidad de ExportarExcelCruceAction.php.
- Utilizar una librería PHP compatible con Excel (ej. PhpSpreadsheet) en modo streaming para generar la hoja del reporte, soportando altos volúmenes en la opción "Todas las fechas".
- El Catálogo de Áreas y Carreras se consulta exclusivamente durante esta acción para enriquecer las columnas correspondientes.

---

### US-006: Gestión de Catálogo de Áreas y Carreras

**As a** Administrador de la academia Vonex
**I want** subir y mantener un catálogo independiente de Áreas y Carreras de la UNMSM
**So that** los reportes Excel exportados puedan ser enriquecidos con esta información oficial sin acoplar el flujo de cruce de ingresantes
...
- [ ] **AC-017:** Dado un archivo CSV del catálogo oficial, cuando el administrador lo sube al módulo independiente, entonces el sistema valida los Magic Bytes, sanitiza los campos para evitar inyección de fórmulas y actualiza la tabla `catalogo_areas_carreras` mediante un *upsert*, sin afectar los lotes de ingresantes previamente procesados.
- [ ] **AC-018:** Dado que se ejecuta el proceso de cruce exacto o difuso, cuando el sistema evalúa a los ingresantes, entonces NO se consulta ni se utiliza el catálogo de áreas y carreras en absoluto, reservando su uso exclusivamente para la exportación final a Excel.

#### Technical Notes
- La tabla subyacente es `catalogo_areas_carreras`.
- El formato de carga está definido en el supuesto **A-06**.
...
### NFR-007: Integridad y Sanitización de Datos (Security)
**Requirement:** El sistema debe prevenir cualquier corrupción en la base de datos o ejecución maliciosa en reportes generados. Todo input de texto debe sanitizarse neutralizando prefijos de fórmulas (`=`, `+`, `-`, `@`). Además, toda interacción con PostgreSQL debe realizarse mediante Prepared Statements usando el ORM Eloquent, aislando absolutamente la estructura SQL del contenido textual de los datos.
**Traces to:** Art. 3 de la Constitución — Data Protection (Sanitización de Exportaciones); US-001 (AC-004b).
**Verification:** (a) Test unitario inyectando caracteres especiales (ej. `; DROP TABLE;`, `' OR '1'='1`) en un CSV y validando que se persisten como cadenas de texto inofensivas. (b) Validar que los Excel exportados anteponen comillas simples a los datos que inician con `=`, `+`, `-`, o `@`.
...
| Catálogo de Áreas y Carreras | Módulo maestro independiente que almacena la relación oficial entre EAP (Escuela Académico Profesional), Área y Carrera de la UNMSM, usado solo en exportación | Dominio de negocio |
| Contrato de Columnas | Definición inmutable de la estructura, orden y cabeceras de la única hoja del reporte Excel exportado | Técnico |
...
| A-06 | El formato de carga del catálogo de Áreas y Carreras (US-006) tiene una estructura mínima de 3 columnas obligatorias: `EAP_ORIGEN`, `AREA_OFICIAL`, `CARRERA_OFICIAL`. | Si el catálogo oficial cambia esta estructura y no incluye el `EAP_ORIGEN`, no habrá clave primaria para cruzar con el campo `EAP` del archivo de ingresantes durante la exportación a Excel, lo que generará `SIN MAPEAR` masivos. |
...
- [x] ✅ [NC-1 — RESUELTO]: ¿El filtro por ALCANZO VACANTE se aplica sobre el valor normalizado (mayúsculas, sin tildes) o sobre el valor crudo del CSV?
...
| 2.6.0 | 2026-07-03 | Equipo V2 | Actualización por impacto de Constitución v2.6.0: US-005 ajustado a exportación exclusiva en Excel, selector de fechas y Contrato de Columnas. Incorporación de US-006 (Gestión del Catálogo). Glosario ampliado y agregado A-06. Deduplicación por CODIGO+OBSERVACION en US-001. |

---

## External References

Fuentes externas consultadas durante la elaboración de esta especificación. Ver source-verification.instructions.md para el flujo DETECT→FETCH→IMPLEMENT→CITE.
...
| Constitución del Proyecto Vonex v2.6.0 (constitution.md) | 2026-07-03 | Art. 2-7 | Fuente de verdad para estándares de calidad, jerarquía de estados y límites del proyecto |
